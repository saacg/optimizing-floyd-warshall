...
#define DISTANCE 20           // determines max weight of the edges  
...
int main(int argc, char* argv[]){
  ...
  void blocked_fw(matrix_ptr G, matrix_ptr pi, int bsize);
...
/*
// intialize an incomplete graph with parameterized density
...
void blocked_fw(matrix_ptr G, matrix_ptr pi, int bsize){
  ...
  data_t* distanceMatrix = get_matrix_start(G);
  data_t* predMatrix = get_matrix_start(pi);
  long int i, j, k, ii, jj, en;

  en = bsize * (vertices / bsize);
