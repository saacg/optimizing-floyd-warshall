/*************************************************************************/
// gcc -pthread -o testPthreadslock pthreads_lock_FW.c -lm -lrt
...
int main(int argc, char* argv[]){
  ...
  for (i = 0; i < ITERS; i++) {
    ...
    init_distance_matrix(G,G_pt, BASE+(i+2)*DELTA, DISTANCE); 
  ...
  OPTION++;
  for (i = 0; i < ITERS; i++) {
    // init_distance_matrix(G_pt,BASE+(i+2)*DELTA, DISTANCE);
    init_predecessor(pi_pt,BASE+(i+2)*DELTA);
    clock_gettime(CLOCK_REALTIME, &time1);
    printf("\nhere\n");
    pt_fw(G_pt,pi_pt);
    clock_gettime(CLOCK_REALTIME, &time2);
  ...
  compareResults(pi_pt, pi, VERTICES);
...
void *cb_work(void *threadarg)
{
  ...
  for(k = 0; k < vertices; k++){
	...
	while(flag[i*vertices+k] < k || flag[k*vertices+j] < k){
	  continue;
	}
