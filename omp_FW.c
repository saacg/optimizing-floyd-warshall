/************************************************************************/
// gcc -O1 -fopenmp -o test omp_FW.c -lrt
...
int main(int argc, char* argv[]){
  ...
  void omp_pragma_middle_FW(matrix_ptr G, matrix_ptr pi, int numThreads);
  ...
  for (i = 0; i < ITERS; i++) {
   ...
   omp_pragma_middle_FW(G, pi, NUM_THREADS);
...
void serial_floyd_warshall(matrix_ptr G, matrix_ptr pi){
  ...
  for(k = 0; k < vertices; k++){
	...
	long int currentIndex = i*vertices+j;
	long int currentPath = distanceMatrix[currentIndex];
	long int nextPath = distanceMatrix[i*vertices+k] + distanceMatrix[k*vertices+j];
...
void omp_pragma_outer_FW(matrix_ptr G, matrix_ptr pi, int numThreads){
  long int vertices = get_matrix_length(G);
  data_t* distanceMatrix = get_matrix_start(G);
  data_t* predMatrix = get_matrix_start(pi);
  long int i, j, k, currentIndex, currentPath, nextPath;

  omp_set_num_threads(numThreads);

#pragma omp parallel shared(G, pi, vertices) private(i, j, k, currentIndex, currentPath, nextPath)
  {
#pragma omp for
    for(k = 0; k < vertices; k++){
      for(i = 0; i < vertices; i++){
...
void omp_pragma_inner_FW(matrix_ptr G, matrix_ptr pi, int numThreads){
  long int vertices = get_matrix_length(G);
  data_t* distanceMatrix = get_matrix_start(G);
  data_t* predMatrix = get_matrix_start(pi);
  long int i, j, k, currentIndex, currentPath, nextPath;
